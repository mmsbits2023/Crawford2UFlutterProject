...
exports.registerClient = (async (request, response, next) => {
    try {
       ...
       /* const phoneNumber1 = await ClientDetails.find({ clientPhoneNumber: clientPhoneNumber });
       ...
       console.log("code of agent is", request.data.code);
        ...
        if (pindata1 === clientCodeData ) {*/
            ...
            clientDetailsCheck.alternativeNumber=alternativeNumber;
            ...
            const clientdetails = await clientDetailsCheck.save(async function (error, saveResult) {
                if (error) { throw new Error(error); }
 ...
 /*exports.getAllAgentList=async function (request,response,next){
  ...
  exports.getAllAgentList = async function (request, response, next) {
    try {
        ...
        responseData.data.agentDetailsList = agentDetailsList;
  ...
  //get data by id
  exports.getOneAgentDetailsById=async function (request,response,next){
    try{
       //const {phoneNumber}=request.body;
            
      const agentDetails= await AgentDetails.findById(request.params.id);
...
exports.uplaodImageAgent =async function(request,response,next){
    try{
        upload(request,response,(err)=>{
            if(err){
                ...
                const newImage=new AgentDetails({
                    image:{
                        data:request.file.filename,
                        contentType:`/uploads/${request.file.filename}`
                    }
