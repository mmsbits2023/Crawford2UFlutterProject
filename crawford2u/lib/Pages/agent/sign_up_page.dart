...
FocusNode firstNameFiled=FocusNode();
  ...
  FocusNode pinFiled=FocusNode();
   ...
   Future<void> fetchUnapprovedItems() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:9006/investor/unApproved'));
      // Handle the response data as needed, e.g., update state
    } catch (error) {
      print('Error fetching unapproved items: $error');
    }
  }

  // Function to make the API call
  Future<void> registerUser() async {
    final url = Uri.parse('http://localhost:5052/agent/register');
